...
class Table:
    def __init__(self, table_conf, univ_conf, cursor):
        self.file_name = table_conf['filename']
...
def get_processing_error_handler(error_file):
    errors = csv.DictWriter(open(error_file + ,'w'),['file_name','line_number','error','linedata'])
    def prossing_error_handler(file_name,l, x, error):
        errors.writerow({'file_name':file_name,'line_number':x,'error':error,'linedata':l})
...
def sql_writer(table_class,univ_conf,cursor):
    ...
    try:
        while True:
            ...
            debug_try(univ_conf, table_class.write, processing_error_handler, x, table_class['file_name'], l, key_values)
            table_class.ptime += time.time()
            x+=1
